...
class DietApp:
    ...
    def _refresh_database_table(self):
        ...
        for food_name in sorted(self.nvalues.keys()):
            ...
            self.db_tree.insert("", tk.END, values=(food_name, f"{kcal:.1f}", f"{prot:.1f}", f"{carb:.1f}", f"{grassi:.1f}", f"{prezzo:.2f}"))
    ...
    def ottimizza_piano(self):
        ...
        for s_idx in range(len(slots_to_fill)):
            for nome in eligible_meals:
                ...
                # La porzione può oscillare tra il 40% (0.4) e il 200% (2.0) della ricetta base
        ...
        porzioni_data = self.look_for("porzioni_piano.json", {})
        for s_idx, (d_key, cat) in enumerate(slots_to_fill):
            for nome in eligible_meals:
                if value(scelto[(s_idx, nome)]) == 1:
        ...
        self.record("porzioni_piano.json", porzioni_data)
    ...
    def _render_planner_screen(self):
        ...
        def on_click(event):
            ...
            if region == "cell":
                ...
                if meal_name in meals:
                    porzioni = self.look_for("porzioni_piano.json", {})
        ...
        btn_opt.config(activebackground="#6d28d9")
        ...
        btn_cns.config(activebackground="#0c6680")
